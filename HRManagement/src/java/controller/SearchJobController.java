...
public class SearchJobController extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            int searchSalary = Integer.parseInt(request.getParameter("searchSalary"));
            JobDAO dao = new JobDAO();
            int total = dao.getCountSearch(searchName, searchSalary);
            if (total > 0) {
                int endPage = total / 3;
                if (total % 3 != 0) {
                    endPage++;
                ...
                List<JobDTO> listJob = dao.getSearchJob(searchName, searchSalary, index);
                request.setAttribute("LIST_JOB", listJob);
                request.setAttribute("TAG", index);
                url = SUCCESS;
            }
        } catch (Exception e) {
            log("Error at SearchJobController" + e.toString());
        } finally {
